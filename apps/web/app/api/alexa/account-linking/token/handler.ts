  ...
  return {
    async getLinkedUserEligibility(profileId: string): Promise<LinkedUserEligibilityResult> {
      try {
        ...
        handleError(error, 'createEligibilityClient.getLinkedUserEligibility');
        return {
          errorMessage: error instanceof Error ? error.message : 'Unexpected linked user eligibility lookup failure.',
          row: null,
        };
      }
    },
    async getProfileEligibility(profileId: string): Promise<ProfileEligibilityResult> {
      try {
        ...
        handleError(error, 'createEligibilityClient.getProfileEligibility');
        return {
          errorMessage: error instanceof Error ? error.message : 'Unexpected profile eligibility lookup failure.',
          row: null,
        };
      }
    },
...
async function resolveLinkEligibility(input: ResolveLinkEligibilityInput): Promise<boolean> {
  try {
    ...
    handleError(error, 'createAlexaAccountLinkingTokenHandler.resolveLinkEligibility');
    throw error;
  }
}
...
async function exchangeAuthorizationCode(
  ...
  try {
    isEligible = await resolveLinkEligibility({
    ...
    handleError(error, 'createAlexaAccountLinkingTokenHandler.exchangeAuthorizationCode.resolveEligibility');
    return NextResponse.json(
      createAlexaOauthErrorPayload('temporarily_unavailable', 'The Alexa authorization code could not be validated right now.'),
      {
        headers: createNoStoreHeaders(),
        status: 503,
      },
  ...
  let successPayload: Awaited<ReturnType<typeof createAlexaTokenSuccessPayload>>;

  try {
    successPayload = await createAlexaTokenSuccessPayload({
    ...
    handleError(error, 'createAlexaAccountLinkingTokenHandler.exchangeAuthorizationCode.createSuccessPayload');
    return NextResponse.json(
      createAlexaOauthErrorPayload('temporarily_unavailable', 'The Alexa access token could not be issued right now.'),
      {
        headers: createNoStoreHeaders(),
        status: 503,
      },
...
async function exchangeRefreshToken(
  ...
  try {
    ...
    handleError(error, 'createAlexaAccountLinkingTokenHandler.exchangeRefreshToken.resolveEligibility');
    return NextResponse.json(
      createAlexaOauthErrorPayload('temporarily_unavailable', 'The Alexa refresh token could not be validated right now.'),
      {
        headers: createNoStoreHeaders(),
        status: 503,
      },
  ...
  try {
    ...
    handleError(error, 'createAlexaAccountLinkingTokenHandler.exchangeRefreshToken.createSuccessPayload');
    return NextResponse.json(
      createAlexaOauthErrorPayload('temporarily_unavailable', 'The Alexa refresh token could not be reissued right now.'),
      {
        headers: createNoStoreHeaders(),
        status: 503,
      },
...
export function createDefaultAlexaAccountLinkingTokenHandler(): (request: NextRequest) => Promise<Response> {
